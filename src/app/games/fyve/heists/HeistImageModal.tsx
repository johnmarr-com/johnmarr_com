...
import { uploadFyveImage } from "@/lib/fyve-storage";
...
function storagePath(
  ...
  switch (type) {
    case "background": return `fyve-heists/${heistId}/background-${v()}.jpg`;
    case "target-object": return `fyve-heists/${heistId}/target-object-${v()}.jpg`;
    case "asset": return `fyve-heists/${heistId}/asset-${index}-${v()}.jpg`;
    case "civilian": return `fyve-heists/${heistId}/civilian-${index}-${v()}.jpg`;
    case "bomb": return `fyve-heists/${heistId}/bomb-${v()}.jpg`;
  }
}
...
interface HeistImageModalProps {
  ...
  currentUrl: string;
  /** Storage type for uploadFyveImage */
...
export default function HeistImageModal({
  ...
  const handleFileChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setError(null);
      try {
        const url = await uploadFyveImage(heistId, storageType, file, index);
        setImageUrl(url);
