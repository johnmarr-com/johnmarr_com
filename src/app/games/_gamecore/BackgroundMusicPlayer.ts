...
class BackgroundMusicPlayer {
   ...
   */
  ensurePlaying(): void {
    if (typeof window === "undefined" || !this.currentURL) return;

    // Snapshot current position while context is still alive
    this.snapshotOffset();
    ...
    // Source died silently (iOS suspension, tab backgrounding, etc.)
    if (!this.sourceNode && this.currentURL) {
      const cached = this.bufferCache.get(this.currentURL);
      if (cached) this.startBuffer(cached, this.currentVolume);
    }
  }

  /** Capture current playback position from the running context. */
  private snapshotOffset(): void {
    if (this.ctx && this.sourceNode && this.sourceNode.buffer) {
      const duration = this.sourceNode.buffer.duration;
      if (duration > 0) {
        this.playbackOffset =
          (this.ctx.currentTime - this.startContextTime) % duration;
      }
    }
  }
