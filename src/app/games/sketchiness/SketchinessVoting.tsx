    ...
    <div className="fixed inset-0 z-10 flex flex-col items-center">
      <div className="flex w-full max-w-lg flex-1 flex-col items-center justify-center gap-6 px-6">
        <JMBannerText borderColor="rgba(239, 68, 68, 0.4)">
          <h1 className="px-4 py-2 text-2xl font-black uppercase tracking-wider text-red-400">
              ...
              {humanPlayers
                ...
                .map((uid) => (
                  ...
                  >
                    ...
                    {hasVoted && votes[userId] === uid && (
                      <span className="ml-auto text-sm font-medium text-red-400">Your vote</span>
                    )}
                  </button>
                ))}
            </div>
            ...
            <div className="space-y-1.5">
              {humanPlayers.map((uid) => (
                ...
                >
                  ...
                  <span className="text-base font-bold text-white">{getPlayerName(uid)}</span>
                  <span className="ml-auto text-sm text-white/50">{voteTally[uid] ?? 0} votes</span>
                  {uid === moleId && (
                    <span className="rounded-full bg-red-400/20 px-2.5 py-0.5 text-xs font-bold text-red-400">
                      MOLE
                    </span>
                  )}
                </div>
