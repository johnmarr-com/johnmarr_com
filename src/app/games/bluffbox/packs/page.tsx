...
import BluffPackEditor from "./BluffPackEditor";
import BluffPackBrowser from "./BluffPackBrowser";

type Tab = "create" | "view";

export default function BluffBoxPacksPage() {
  ...
  const [editingPack, setEditingPack] = useState<BluffBoxPack | null>(null);
  const [refreshKey, setRefreshKey] = useState(0);

  useEffect(() => {
    if (!authLoading && !user) {
      router.push("/games/bluffbox");
    }
  ...
  const handleEdit = useCallback((pack: BluffBoxPack) => {
    setEditingPack(pack);
    setTab("create");
  }, []);
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-white">
      <div className="mx-auto max-w-lg px-4 py-6 lg:max-w-3xl xl:max-w-5xl">
        <button
          onClick={() => router.push("/games/bluffbox")}
