...
import BlarfPackEditor from "./BlarfPackEditor";
import BlarfPackBrowser from "./BlarfPackBrowser";

type Tab = "browse" | "create";

export default function BlarfPacksPage() {
  const { user, userTier, isAdmin, isLoading: authLoading } = useAuth();
  const router = useRouter();
  const canCreate = isAdmin || userTier === "pro";

  const [tab, setTab] = useState<Tab>("browse");
  const [editingPack, setEditingPack] = useState<BlarfPack | null>(null);
  const [refreshKey, setRefreshKey] = useState(0);

  useEffect(() => {
    if (!authLoading && !user) {
      router.push("/games/blarf");
    }
  ...
  const handleEdit = useCallback((pack: BlarfPack) => {
    setEditingPack(pack);
    setTab("create");
  }, []);
  ...
  if (authLoading) {
    return (
      <div className="flex min-h-dvh items-center justify-center bg-[#2B4B6F]">
  ...
  return (
    <div className="relative min-h-dvh bg-[#2B4B6F] px-4 py-8">
      <div className="absolute right-4 top-4 z-10">
        <JMCloseCircleButton onClick={() => router.push("/games/blarf")} />
      </div>
      ...
      <div className="mx-auto max-w-2xl">
        <GameSectionHeader
          eyebrow="BLARF!"
          title="Word Packs"
          titleColorClass="text-[#F7D047]"
          eyebrowColorClass="text-[#F7D047]/70"
        />
        ...
        <div className="mt-5 flex rounded-xl border border-white/10 bg-white/5 p-1">
          <button
            className={`flex-1 rounded-lg py-2.5 text-sm font-bold tracking-wide transition ${
              tab === "browse"
                ? "bg-[#C93C3C] text-white shadow-lg"
