...
import { getOfficialHeists } from "@/lib/sevyn-heists";
...
function pickDefaultHeist(heists: SevynHeist[]): SevynHeist | null {
...
async function writeLobbyHeistToSession(
  ...
  await updateDoc(doc(db, "gameSessions", sessionId), {
    sevynLobbyHeistId: heist.id,
    sevynLobbyHeistTitle: heist.title,
    sevynLobbyHeistBgUrl: heist.backgroundImageUrl,
 ...
 * Writes `sevynLobbyHeist*` fields to the session so SevynGame can read them.
 */
export default function HeistLobbySelector({
  sessionId,
  accentColor = "#E84C1E",
}: HeistLobbySelectorProps) {
  const [selected, setSelected] = useState<SevynHeist | null>(null);
