...
import { getOfficialBombs, getMyBombs, getSharedBombs } from "@/lib/sevyn-bombs";
import type { SevynBombEntity } from "../sevynTypes";
...
interface BombPickerProps {
  ...
  onSelect: (bomb: SevynBombEntity) => void;
}
...
export default function BombPicker({ selectedBombId, onSelect }: BombPickerProps) {
  ...
  const [officialList, setOfficialList] = useState<SevynBombEntity[]>([]);
  const [myList, setMyList] = useState<SevynBombEntity[]>([]);
  const [sharedList, setSharedList] = useState<SevynBombEntity[]>([]);
