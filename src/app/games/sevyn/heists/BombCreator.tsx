...
import { createBomb } from "@/lib/sevyn-bombs";
import { uploadBombImage, uploadBombAudio } from "@/lib/sevyn-bomb-storage";
...
interface BombCreatorProps {
  onCreated: (bomb: SevynBombEntity) => void;
  onCancel: () => void;
}
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (imageUrl && isEphemeral) {
        setPersisting(true);
        const headers = await getAIAuthHeaders();
        const storagePath = `sevyn-bombs/${tempId}/bomb-${Date.now().toString(36)}.jpg`;
        const res = await fetch("/api/games/ai", {
          method: "POST",
          headers,
          body: JSON.stringify({
