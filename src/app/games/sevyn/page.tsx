...
import SevynGame from "./SevynGame";
import HeistLobbySelector from "./HeistLobbySelector";

export default function SevynPage() {
  ...
  useEffect(() => {
    getContentBySlug("game", "sevyn").then(setGameData);
  }, []);
  ...
  const landingExtra = canCreate ? (
    <JMProButton
      title="Build Heists"
      onClick={() => router.push("/games/sevyn/heists")}
    />
  ) : null;

  // SEVYN is 4+ players (2 teams of 2+ each)
  ...
  return (
    <GameLandingPage
      ...
      lobbyCanStart={({ session }: { session: GameSession }) =>
        !!(session as unknown as Record<string, unknown>)["sevynLobbyHeistId"]
      }
