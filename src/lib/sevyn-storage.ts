...
/**
 * SEVYN — Firebase Storage helpers for heist images.
 *
 * Storage paths:
 *   sevyn-heists/{heistId}/background.jpg
 *   sevyn-heists/{heistId}/target-object.jpg
 *   sevyn-heists/{heistId}/asset-{index}.jpg
 *   sevyn-heists/{heistId}/civilian-{index}.jpg
 *   sevyn-heists/{heistId}/bomb.jpg
 */
...
function storagePath(
  ...
  switch (type) {
    case "background":
      return `sevyn-heists/${heistId}/background-${v()}.jpg`;
    case "target-object":
      return `sevyn-heists/${heistId}/target-object-${v()}.jpg`;
    case "asset":
      return `sevyn-heists/${heistId}/asset-${index}-${v()}.jpg`;
    case "civilian":
      return `sevyn-heists/${heistId}/civilian-${index}-${v()}.jpg`;
    case "bomb":
      return `sevyn-heists/${heistId}/bomb-${v()}.jpg`;
  }
}
 ...
 */
export async function uploadSevynImage(
  heistId: string,
  type: "background" | "target-object" | "asset" | "civilian" | "bomb",
  blob: Blob,
  index?: number,
...
export async function deleteSevynHeistImages(heistId: string): Promise<void> {
  ...
  const folderRef = ref(storage, `sevyn-heists/${heistId}`);
  try {
