...
export const WORDONKULOUS_IMAGE_ACCEPT = "image/jpeg,image/png,image/webp";

const MAX_UPLOAD_BYTES = 12 * 1024 * 1024;

/** Returns an error message if invalid, otherwise `null`. */
export function validateWordonkulousImageFile(file: File): string | null {
...
/**
 * Upload a Wordonkulous pack cover image to Firebase Storage.
 * Resizes to 720×720 first, then uploads and returns a permanent public URL.
 */
export async function uploadWordonkulousCover(
  packId: string,
  blob: Blob,
): Promise<string> {
  ...
  const path = `wordonkulous-packs/${packId}/cover.jpg`;
 ...
 */
export async function fetchWordonkulousImageAsBlob(url: string): Promise<Blob> {
