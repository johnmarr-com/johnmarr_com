...
/**
 * SEVYN — Firebase Storage helpers for bomb images/audio.
 *
 * Storage paths:
 *   sevyn-bombs/{bombId}/bomb.jpg
 *   sevyn-bombs/{bombId}/bomb-audio.mp3
 */
...
export async function uploadBombImage(
  ...
  const path = `sevyn-bombs/${bombId}/bomb.jpg`;
...
export async function uploadBombAudio(
  ...
  const path = `sevyn-bombs/${bombId}/bomb-audio.mp3`;
  const storageRef = ref(storage, path);
...
export async function deleteBombFiles(bombId: string): Promise<void> {
  ...
  const folderRef = ref(storage, `sevyn-bombs/${bombId}`);
  try {
